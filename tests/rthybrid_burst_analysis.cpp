...
TEST(RTHYBRID_BURST_ANALYSIS, DescriptorShape)
{
    const auto *descriptor = rtsyn_rthybrid_burst_analysis_get_descriptor();
    ASSERT_NE(descriptor, nullptr);
    EXPECT_STREQ(descriptor->name, "rthybrid_burst_analysis");
    EXPECT_EQ(descriptor->node_type, RTSYN_ABI_NODE_PLUGIN);
    EXPECT_EQ(descriptor->port_count, 4U);
    EXPECT_EQ(descriptor->param_count, 1U);
    EXPECT_EQ(descriptor->state_count, 6U);
}

TEST(RTHYBRID_BURST_ANALYSIS, ProcessesAndPublishesFiniteOutput)
{
    const auto *descriptor = rtsyn_rthybrid_burst_analysis_get_descriptor();
...
TEST(RTHYBRID_BURST_ANALYSIS, StateAndInvalidAccess)
{
    const auto *descriptor = rtsyn_rthybrid_burst_analysis_get_descriptor();
    void *instance = nullptr;
    double value = 1.0;
